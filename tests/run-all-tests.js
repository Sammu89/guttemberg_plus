...
const runAllTests = () => {
	...
	console.log(
		'║      WordPress Gutenberg Blocks - Phase 5 Integration Tests   ║'
	);
	console.log(
		'╚════════════════════════════════════════════════════════════════╝'
	);
	...
	try {
		...
		console.error(
			'\n❌ Event Isolation tests failed with error:',
			error.message
		);
	...
	try {
		...
		console.error(
			'\n❌ Accessibility tests failed with error:',
			error.message
		);
	...
	const totalPassed =
		results.eventIsolation.passed +
		results.performance.passed +
		results.accessibility.passed;
	const totalFailed =
		results.eventIsolation.failed +
	...
	console.log(
		'│ Test Suite              │ Passed │ Failed │ Total │ Success  │'
	);
	console.log(
		'├─────────────────────────┼────────┼────────┼───────┼──────────┤'
	);
	...
	console.log(
		`│ Event Isolation         │ ${ String(
			results.eventIsolation.passed
		).padEnd( 6 ) } │ ${ String( results.eventIsolation.failed ).padEnd(
			6
		) } │ ${ String( results.eventIsolation.total ).padEnd(
			5
		) } │ ${ String( eventSuccess ).padEnd( 7 ) }% │`
	);
	console.log(
		`│ Performance             │ ${ String(
			results.performance.passed
		).padEnd( 6 ) } │ ${ String( results.performance.failed ).padEnd(
			6
		) } │ ${ String( results.performance.total ).padEnd( 5 ) } │ ${ String(
			perfSuccess
		).padEnd( 7 ) }% │`
	);
	console.log(
		`│ Accessibility           │ ${ String(
			results.accessibility.passed
		).padEnd( 6 ) } │ ${ String( results.accessibility.failed ).padEnd(
			6
		) } │ ${ String( results.accessibility.total ).padEnd(
			5
		) } │ ${ String( a11ySuccess ).padEnd( 7 ) }% │`
	);
	console.log(
		'├─────────────────────────┼────────┼────────┼───────┼──────────┤'
	);
	...
	console.log(
		`│ TOTAL                   │ ${ String( totalPassed ).padEnd(
			6
		) } │ ${ String( totalFailed ).padEnd( 6 ) } │ ${ String(
			totalTests
		).padEnd( 5 ) } │ ${ String( overallSuccess ).padEnd( 7 ) }% │`
	);
	console.log(
		'└─────────────────────────┴────────┴────────┴───────┴──────────┘'
	);
	...
	if ( totalFailed === 0 ) {
		...
		console.log(
			'║  All 3 blocks ready for production deployment                  ║'
		);
	} else {
		console.log(
	...
	console.log(
		'  • Block Sizes: Accordion 28.4KB | Tabs 36.7KB | TOC 23.9KB'
	);
