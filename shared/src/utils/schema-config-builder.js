 ...
 * @returns {Object} Configuration with structure: { main: { attributes: {...} }, divider: { attributes: {...} } }
 *
 * @example
 * // Build border config for accordion
 * import accordionSchema from '../../../schemas/accordion.json';
 * const borderConfig = buildBorderConfig(accordionSchema, 'accordion');
 * // Returns: {
 * //   main: {
 * //     attributes: {
 ...
 *
 * @example
 * // Build border config for tabs (has dividerThickness, dividerStyle, dividerColor)
 * import tabsSchema from '../../../schemas/tabs.json';
 * const borderConfig = buildBorderConfig(tabsSchema, 'tabs');
 */
export function buildBorderConfig( schema, blockType ) {
	// Get border attributes from 'border' and 'border-divider' groups
	const mainConfig = getPanelConfig( schema, 'border' );
	const dividerConfig = getPanelConfig( schema, 'border-divider' );
	...
	const dividerAttrsList = { ...dividerConfig.attributes };

	// Also check colors group for divider-related color attributes
	for ( const [ attrName, attrConfig ] of Object.entries( colorsConfig.attributes ) ) {
		const lowerName = attrName.toLowerCase();
		if ( lowerName.includes( 'divider' ) ) {
			dividerAttrsList[ attrName ] = attrConfig;
		}
	}

	if ( Object.keys( dividerAttrsList ).length > 0 ) {
		Object.assign( dividerAttributes, mapBorderAttributes( dividerAttrsList ) );
	}

	return {
		main: { attributes: mainAttributes, title: `${ blockType.charAt( 0 ).toUpperCase() + blockType.slice( 1 ) } Border` },
		divider: Object.keys( dividerAttributes ).length > 0 ? { attributes: dividerAttributes, title: 'Divider Border' } : null,
	};
}

/**
 * Map border attributes to roles (color, thickness, style, radius, shadow, shadowHover)
 * @param {Object} attributes - Border-related attributes from schema
 * @returns {Object} Mapped attributes organized by role
 */
function mapBorderAttributes( attributes ) {
	...
	for ( const [ attrName, attrConfig ] of Object.entries( attributes ) ) {
		...
		// Match attribute names to roles
		if ( lowerName.includes( 'color' ) && ! lowerName.includes( 'thickness' ) && ! lowerName.includes( 'style' ) ) {
			mapped.color = { name: attrName, ...attrConfig };
		} else if ( lowerName.includes( 'thickness' ) || lowerName.includes( 'width' ) ) {
			mapped.thickness = { name: attrName, ...attrConfig };
		} else if ( lowerName.includes( 'style' ) ) {
			mapped.style = { name: attrName, ...attrConfig };
		} else if ( lowerName.includes( 'radius' ) ) {
			mapped.radius = { name: attrName, ...attrConfig };
		} else if ( lowerName.includes( 'shadow' ) && lowerName.includes( 'hover' ) ) {
			mapped.shadowHover = { name: attrName, ...attrConfig };
		} else if ( lowerName.includes( 'shadow' ) ) {
			mapped.shadow = { name: attrName, ...attrConfig };
		}
	}
