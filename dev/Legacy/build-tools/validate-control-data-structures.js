...
function main() {
	...
	console.log(
		`${ colors.dim }Checking ${ allFiles.length } control files...${ colors.reset }\n`
	);
	...
	if ( info.length > 0 ) {
		console.log(
			`${ colors.green }${ colors.bold }✓ DOCUMENTED CONTROLS (${ info.length }):${ colors.reset }\n`
		);
		info.forEach( ( item ) => {
			const status = item.complete ? '✓' : '○';
			const color = item.complete ? colors.green : colors.yellow;
			console.log(
				`  ${ color }${ status }${ colors.reset } ${ item.file } ${ colors.dim }(${ item.pattern })${ colors.reset }`
			);
		} );
		console.log();
	}
	...
	console.log(
		`  Critical controls: ${ colors.cyan }${ CRITICAL_CONTROLS.length }${ colors.reset }`
	);
	console.log( `  Documented controls: ${ colors.green }${ info.length }${ colors.reset }` );
	console.log(
		`  Complete documentation: ${ colors.green }${ info.filter( ( i ) => i.complete ).length }${
			colors.reset
		}`
	);
	...
	if ( errors.length > 0 ) {
		...
		console.log(
			`${ colors.yellow }${ colors.bold }⚠️  Non-critical issues found${ colors.reset }`
		);
		console.log(
			`${ colors.dim }Consider addressing warnings for better standardization.${ colors.reset }\n`
		);
		process.exit( 0 );
	} else {
		console.log(
			`${ colors.green }${ colors.bold }✅ All controls properly documented!${ colors.reset }\n`
		);
		process.exit( 0 );
	}
}
