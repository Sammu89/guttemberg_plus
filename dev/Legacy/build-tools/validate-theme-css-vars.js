...
async function main() {
	...
	if ( files.length === 0 ) {
		console.log(
			'No theme CSS files provided. Set GUTPLUS_THEME_CSS_DIR or GUTPLUS_THEME_CSS_PATHS.'
		);
		return;
	}
	...
	const [ { CSS_VAR_MAPPINGS }, { getPropertyScale } ] = await Promise.all( [
		import(
			pathToFileURL( path.join( ROOT, 'shared/src/config/css-var-mappings-generated.js' ) )
				.href
		),
		import(
	...
	if ( errors.length > 0 ) {
		...
		errors.forEach( ( err ) => {
			console.error(
				`  ${ colors.red }${ err.filePath }${ colors.reset }: ${ err.cssVarName } = ${ err.value }`
			);
		} );
		process.exit( 1 );
	}
	...
	console.log(
		`${ colors.green }${ colors.bold }✓ Theme CSS variables validated${ colors.reset }`
	);
}
