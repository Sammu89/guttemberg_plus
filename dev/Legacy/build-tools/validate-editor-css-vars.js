...
function validateEditFile( blockType ) {
	...
	const autoGenStart = content.indexOf(
	...
	for ( const pattern of BAD_PATTERNS ) {
		...
		for ( const match of matches ) {
			...
			if (
				autoGenStart !== -1 &&
				autoGenEnd !== -1 &&
				matchIndex > autoGenStart &&
				matchIndex < autoGenEnd
			) {
	...
	if ( hasRootStyles || hasBlockProps ) {
		...
		const spreadsVars =
			/rootStyles\s*=\s*\{[^}]*\.\.\.(cssVars|editorVars|editorCSSVars)/s.test( content ) ||
...
function main() {
	...
	console.log(
		'\x1b[90mChecking that edit.js uses CSS variables instead of direct properties\x1b[0m\n'
	);
	...
	for ( const blockType of BLOCKS ) {
			...
			result.errors.forEach( ( error, index ) => {
				...
				if ( error.details && error.details.length > 0 ) {
					...
					error.details.forEach( ( detail, i ) => {
						console.log(
							`        ${ i + 1 }. ${ detail.property }: effectiveValues.${
								detail.attrName
							} (line ${ detail.line })`
						);
						if ( detail.snippet && i === 0 ) {
							// Show snippet for first issue only
		...
		if ( result.warnings.length > 0 ) {
			console.log(
				`\x1b[33m⚠️  ${ blockType }: ${ result.warnings.length } warning(s)\x1b[0m`
			);
	...
	console.log(
		'\x1b[90mThemeable properties that should use CSS vars: ' +
			THEMEABLE_PROPERTIES.slice( 0, 5 ).join( ', ' ) +
			'...\x1b[0m'
	);
	...
	for ( const [ blockType, result ] of Object.entries( results ) ) {
		for ( const error of result.errors ) {
			if (
				error.issue.includes( 'Missing CSS variable generation function' ) ||
	...
	if ( criticalErrors.length > 0 ) {
		...
		console.log(
			'\x1b[90mThe following blocks are missing CSS variable implementation:\x1b[0m\n'
		);
		...
		console.log(
			'\x1b[90mSome direct CSS property usage detected. Consider migrating to CSS variables.\x1b[0m\n'
		);
