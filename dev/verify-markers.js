...
files.forEach( ( file ) => {
	...
	// Count occurrences
	const startCount = (
		content.match( new RegExp( startMarker.replace( /[.*+?^${}()|[\]\\]/g, '\\$&' ), 'g' ) ) ||
		[]
	).length;
	const endCount = (
		content.match( new RegExp( endMarker.replace( /[.*+?^${}()|[\]\\]/g, '\\$&' ), 'g' ) ) || []
	).length;
	...
	const status =
		hasStart && hasEnd && startCount === 1 && endCount === 1 ? '✓ VALID' : '✗ INVALID';
	...
	console.log(
		`  Start Marker: ${ hasStart ? 'Found' : 'Missing' } (${ startCount } occurrence${
			startCount !== 1 ? 's' : ''
		})`
	);
	console.log(
		`  End Marker: ${ hasEnd ? 'Found' : 'Missing' } (${ endCount } occurrence${
			endCount !== 1 ? 's' : ''
		})`
	);
