 ...
 * 1. Schema → Code: All attributes exist and are accessed in both files
 * 2. Code → Schema: All accessed attributes are defined in schema
 * 3. Structure → Code: All elements are rendered in generated code
...
function validateBlock(blockType) {
	...
	// Load autogenerated schema (has expanded icon-panel attributes)
	const schemaPath = path.join(SCHEMAS_DIR, `${blockType}-schema-autogenerated.json`);
	if (!fs.existsSync(schemaPath)) {
		// Fall back to source schema if autogenerated doesn't exist
		const sourceSchemaPath = path.join(SCHEMAS_DIR, `${blockType}.json`);
		if (!fs.existsSync(sourceSchemaPath)) {
			errors.push(`Schema not found: ${schemaPath} or ${sourceSchemaPath}`);
	...
	// ============================================================
	// VALIDATION 1: Code → Schema (all accessed attrs exist)
	// ============================================================
	...
	// ============================================================
	// VALIDATION 2: Schema → Code (all attrs are used somewhere)
	// ============================================================
	...
	// ============================================================
	// VALIDATION 4: Structure → Code (all elements rendered)
	// ============================================================
...
function validateAll(options = {}) {
	...
	// Print results
	console.log('\n╔════════════════════════════════════════════════════════════╗');
	console.log('║  Schema ↔ Code Sync Validation                            ║');
	console.log('╚════════════════════════════════════════════════════════════╝\n');
	...
	results.forEach(({ blockType, errors, warnings }) => {
		...
		if (hasIssues) {
			...
			console.log(`✅ ${blockType}: All attributes synced correctly`);
		}
	});
