 ...
 * @return {Object} { valid: boolean, invalidClasses: string[] }
 */
 ...
 * @return {Object} { hasCircular: boolean, circularPath: string[] }
 */
...
function validateBlock( blockType ) {
	...
	if ( ! attributes.attributes ) {
		...
		Object.entries( attributes.attributes ).forEach( ( [ attrName, attr ] ) => {
			if ( ! attr.dependsOn && ! attr.variants ) {
				return;
			}

			const variants =
				attr.variants && typeof attr.variants === 'object' ? attr.variants : null;
			...
			if ( optionValues.length > 0 ) {
				...
				const unexpected = nonDefaultVariantKeys.filter(
					( key ) => ! optionValues.includes( key )
				);
		...
		Object.entries( attributes.attributes ).forEach( ( [ attrName, attr ] ) => {
			if ( attr.themeable && attr.appliesTo ) {
				...
				appliesToElements.forEach( ( elementId ) => {
					...
					if ( element ) {
						if (
							! element.appliesStyles ||
							! element.appliesStyles.includes( attrName )
						) {
							warnings.push(
 ...
 * @return {string[]} Array of similar element IDs
 */
 ...
 * @return {string[]} Array of similar attribute names
 */
...
function reportResults( blockType, errors, warnings ) {
	...
	if ( errors.length > 0 ) {
		console.log(
			`\n❌ ${ blockType }: FAILED (${ errors.length } error${
				errors.length > 1 ? 's' : ''
			})`
		);
	...
	if ( warnings.length > 0 ) {
		console.log(
			`⚠️  ${ blockType }: ${ warnings.length } warning${ warnings.length > 1 ? 's' : '' }`
		);
...
function printSummary( results ) {
	...
	results.forEach( ( result ) => {
		...
		console.log(
			`${ status } ${ result.blockType.padEnd( 12 ) } - ${
