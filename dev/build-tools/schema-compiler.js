...
const OUTPUT_DIRS = {
	...
	cssGenerated: path.join( ROOT_DIR, 'css', 'generated' ),
};
...
async function compile( options = {} ) {
	...
	try {
		...
		// ========================================
		// SCSS Generation Phase
		// ========================================
		// Generate comprehensive SCSS files for each block
		try {
			const { parseHTMLTemplate } = require( path.join( ROOT_DIR, 'schemas', 'parser', 'html-parser' ) );
			const { mergeStructureIntoSchema } = require( path.join( ROOT_DIR, 'schemas', 'parser', 'schema-merger' ) );
			const { createComprehensiveSchema, writeComprehensive } = require( path.join( ROOT_DIR, 'schemas', 'parser', 'main-orchestrator' ) );
			const { generateAndWriteSCSS } = require( path.join( ROOT_DIR, 'schemas', 'parser', 'scss-generator' ) );

			for ( const blockType of BLOCKS ) {
				try {
					// Load HTML structure
					const structurePath = path.join( SCHEMAS_DIR, `${ blockType }-structure.html` );
					if ( ! fs.existsSync( structurePath ) ) {
						results.errors.push( `SCSS generation skipped for ${ blockType }: HTML structure file not found` );
						continue;
					}
					...
					const merged = mergeStructureIntoSchema( schemas[ blockType ], htmlStructure );

					// Create comprehensive schema
					const comprehensive = createComprehensiveSchema( merged );

					// Write structure mapping JSON file (writeComprehensive handles this automatically)
					// Note: writeComprehensive writes both the comprehensive schema AND the structure mapping
					// We pass a temp output path that won't be used (it writes to auto-generated paths)
					const comprehensiveSchemaPath = path.join( SCHEMAS_DIR, `${ blockType }-comprehensive-autogenerated.json` );
					writeComprehensive( comprehensiveSchemaPath, comprehensive );

					// Generate SCSS file
					const outputPath = path.join( OUTPUT_DIRS.cssGenerated, `${ blockType }_variables.scss` );
					generateAndWriteSCSS( comprehensive, outputPath );

					results.files.push( { path: outputPath, lines: fs.readFileSync( outputPath, 'utf8' ).split( '\n' ).length } );
				} catch ( error ) {
					results.errors.push( `SCSS generation failed for ${ blockType }: ${ error.message }` );
				}
			}
		} catch ( error ) {
			results.errors.push( `SCSS generation phase failed: ${ error.message }` );
		}
