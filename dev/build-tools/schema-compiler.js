...
function generateInlineStylesFunction(schema, blockType) {
  ...
  for (const [attrName, attr] of Object.entries(schema.attributes)) {
    ...
    if (!selectorKey) {
      ...
      const knownSkipElements = ['tocLink', 'tocList', 'link', 'list', 'collapseIcon', 'tabButton', 'level1Link', 'level2Link', 'level3PlusLink', 'h1Link', 'h2Link', 'h3Link', 'h4Link', 'h5Link', 'h6Link', 'numberingStyle'];
...
async function compile(options = {}) {
  ...
  try {
    ...
    for (const [blockType, schema] of Object.entries(schemas)) {
      const expandedSchemaPath = path.join(__dirname, '..', 'schemas', `${blockType}-schema-autogenerated.json`);
  ...
  if (hasErrors && warnOnly) {
    console.log(`⚠️  Schema compilation completed with ${results.errors.length} warning(s) (${elapsed}ms)\n`);
    return results;
  }
...
function isWarnOnly() {
  return process.argv.includes('--warn-only') ||
