...
function findUsedAttributes(code, schemaAttributes) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		for (const attr of schemaAttributes) {
			...
			if (FALSE_POSITIVE_NAMES.has(attr)) continue;

			// Skip if this is a locally declared variable
			if (localVars.has(attr)) continue;
...
function main() {
	...
	if (allErrors.length > 0) {
		...
		for (const error of allErrors) {
			...
			for (const usage of error.usages) {
				console.log(`    ${colors.gray}Line ${usage.line}: ${usage.context}${colors.reset}`);
			}
	...
	console.log(`${colors.green}${colors.bold}✓ Attribute destructuring validated${colors.reset}\n`);
}
