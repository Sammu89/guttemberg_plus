		...
		while ((application = applicationRegex.exec(editCode)) !== null) {
			...
			if (lastMatch) {
				...
				const isParentElement =
					(className.includes('accordion-title') && !className.includes('accordion-title-text')) ||
					(className.includes('tab-button') && !className.includes('tab-button-text')) ||
					(className.includes('toc-title') && !className.includes('toc-title-text'));
...
function validateIconTextDecorationReset(blockName, editCode, editPath) {
	...
	while ((match = iconStyleRegex.exec(editCode)) !== null) {
		...
		const hasParentTextDecoration =
			iconContext.match(/style=\{[^}]*\.\.\.(\w*[Ff]ormatting[Ss]tyles)[^}]*\}[^<]*\{[^}]*icon/s) ||
			iconContext.match(/style=\{[^}]*textDecoration[^}]*\}[^<]*\{[^}]*icon/s);

		if (hasParentTextDecoration && !hasTextDecorationReset && !styleContent.includes('textDecoration')) {
...
function validateTransformProperty(blockName, editCode, editPath) {
	...
	while ((match = transformRegex.exec(editCode)) !== null) {
		...
		const hasFunctionWrapper = /^['"`]?(rotate|scale|translate|skew|matrix|perspective)\s*\(/.test(
			transformValue
		);
		...
		const templateHasFunction = isTemplateLiteral && /rotate|scale|translate/.test(transformValue);
		...
		if (isRawValue && !hasFunctionWrapper && !templateHasFunction) {
			...
			errors.push({
				...
				fix:
					transformValue.includes('effectiveValues') || transformValue.includes('attributes')
...
function validateClassNameParity(blockName, editCode, saveCode, editPath, savePath) {
	...
	const structuralElements = {
		accordion: ['accordion-title', 'accordion-title-text', 'accordion-icon', 'accordion-content'],
	...
	for (const elementClass of elementsToCheck) {
		...
		const editClassRegex = new RegExp(`className=\\{?\\s*["\`']([^"\`']*${elementClass}[^"\`']*)["\`']`, 'g');
		...
		const saveLiteralRegex = new RegExp(`className=\\{?\\s*["\`']([^"\`']*${elementClass}[^"\`']*)["\`']`, 'g');
		const saveExpressionRegex = new RegExp(`className=\\{[^}]*['"\`]([^'"\`]*${elementClass}[^'"\`]*)['"\`]`, 'g');
		...
		if (elementClass.includes('-icon')) {
			...
			saveMatches = saveMatches.filter(match => {
				...
				return classes.some(cls =>
					cls === elementClass ||
					(cls.startsWith(elementClass + '-') && !cls.endsWith('-wrapper') && !cls.endsWith('-slot'))
				);
			});
		}
		...
		if (editMatches.length > 0 && saveMatches.length > 0) {
			...
			if (
				...
				if (editClass.includes('${') && saveClass.includes('${')) {
					...
					if (editClass.startsWith('tab-button${') && saveClass.startsWith('tab-button${')) {
			...
			// Compare class lists
			const editClasses = new Set(editMatches[0][1].split(/\s+/).filter((c) => c));
			const saveClasses = new Set(saveMatches[0][1].split(/\s+/).filter((c) => c));
...
function validateInlineStyleParity(blockName, editCode, saveCode, editPath, savePath) {
	...
	for (const elementClass of elementsToCheck) {
		...
		const hasCssVars = saveCode.includes('buildFrontendCssVars') || saveCode.includes('data-style');
	...
	for (let i = 0; i < Math.min(editHeadings.length, saveHeadings.length); i++) {
		...
		if (editHasMargin !== saveHasMargin) {
			...
			errors.push({
				...
				expected: editHasMargin ? 'Heading with margin in both files' : 'No margin in both files',
...
function validateCSSVariableConsistency(blockName, styleCode, stylePath) {
	...
	const editorVarsPath = path.join(ROOT_DIR, `shared/src/styles/${blockName}-css-vars-generated.js`);
	...
	const editorVars = new Set([...editorVarsCode.matchAll(varPattern)].map((m) => m[0].toLowerCase()));
	const frontendVars = new Set([...frontendVarsCode.matchAll(varPattern)].map((m) => m[0].toLowerCase()));
...
function main() {
	...
	for (const blockName of BLOCKS) {
		...
		if (result.skipped) {
			console.log(`${colors.gray}⊘ ${blockName.toUpperCase()} - Skipped (files not found)${colors.reset}`);
			continue;
		}
		...
		if (result.errors.length > 0) {
			console.log(
				`${colors.red}✗ ${blockName.toUpperCase()} - ${result.errors.length} issue(s) found${colors.reset}\n`
			);
			...
			for (const [rule, ruleErrors] of Object.entries(errorsByRule)) {
				...
				for (const error of ruleErrors) {
					...
					console.log(`    ${colors.gray}Expected:${colors.reset} ${error.expected}`);
					console.log(`    ${colors.gray}Found:${colors.reset}    ${error.found}`);
					console.log(`    ${colors.green}Fix:${colors.reset}      ${error.fix}`);
					if (error.context) {
						console.log(`    ${colors.gray}Context:${colors.reset}  ${error.context}`);
					}
					console.log('');
				}
			}
			...
			console.log(`${colors.green}✓ ${blockName.toUpperCase()} - No issues found${colors.reset}\n`);
		}
	}
	...
	console.log(
		`${colors.bold}Summary: ${totalErrors > 0 ? colors.red : colors.green}${totalErrors} total issue(s)${colors.reset}\n`
	);

	if (totalErrors > 0) {
		console.log(`${colors.red}${colors.bold}Validation failed.${colors.reset} Please fix the issues above.\n`);
		process.exit(1);
	} else {
		console.log(
