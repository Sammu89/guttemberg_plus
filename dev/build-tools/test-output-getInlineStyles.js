...
const getInlineStyles = () => {
	...
	if (
		effectiveValues.titleBackgroundColor !== undefined &&
		effectiveValues.titleBackgroundColor !== null
	) {
		titleStyles.backgroundColor = effectiveValues.titleBackgroundColor;
	}
	if (
		effectiveValues.hoverTitleColor !== undefined &&
		effectiveValues.hoverTitleColor !== null
	) {
		titleStyles.color = effectiveValues.hoverTitleColor;
	}
	if (
		effectiveValues.hoverTitleBackgroundColor !== undefined &&
		effectiveValues.hoverTitleBackgroundColor !== null
	) {
	...
	if (
		effectiveValues.titleFontWeight !== undefined &&
		effectiveValues.titleFontWeight !== null
	) {
	...
	if (
		effectiveValues.titleTextTransform !== undefined &&
		effectiveValues.titleTextTransform !== null
	) {
		titleStyles.textTransform = effectiveValues.titleTextTransform;
	}
	if (
		effectiveValues.titleTextDecoration !== undefined &&
		effectiveValues.titleTextDecoration !== null
	) {
	...
	if ( effectiveValues.dividerWidth !== undefined && effectiveValues.dividerWidth > 0 ) {
		contentStyles.borderTop = `${ effectiveValues.dividerWidth || 1 }px ${
	...
	if (
		effectiveValues.contentBackgroundColor !== undefined &&
		effectiveValues.contentBackgroundColor !== null
	) {
		contentStyles.backgroundColor = effectiveValues.contentBackgroundColor;
	}
	if (
		effectiveValues.contentFontSize !== undefined &&
		effectiveValues.contentFontSize !== null
	) {
	...
	// border shorthand
	containerStyles.border = `${ effectiveValues.borderWidth || 1 }px ${
		effectiveValues.borderStyle || 'solid'
	} ${ effectiveValues.borderColor || '#dddddd' }`;
	if ( effectiveValues.borderRadius ) {
		const br = effectiveValues.borderRadius;
		containerStyles.borderRadius = `${ br.topLeft || 0 }px ${ br.topRight || 0 }px ${
			br.bottomRight || 0
		}px ${ br.bottomLeft || 0 }px`;
	}
