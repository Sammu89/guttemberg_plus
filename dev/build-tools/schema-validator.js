...
function validateJSONSyntax(filePath) {
  ...
  try {
    ...
    errors.push({
      ...
      location: error.message.match(/position (\d+)/) ? `position ${error.message.match(/position (\d+)/)[1]}` : 'unknown',
    });
  }
...
function validateAttribute(attrName, attr, blockType) {
  ...
  if (attr.type && !VALID_TYPES.includes(attr.type)) {
    errors.push({
      type: 'invalid_value',
      message: `Attribute "${attrName}" has invalid type: "${attr.type}". Valid types: ${VALID_TYPES.join(', ')}`,
  ...
  if (attr.themeable === false) {
    if (!attr.reason) {
      ...
      warnings.push({
        type: 'unknown_reason',
        message: `Attribute "${attrName}" has unknown reason: "${attr.reason}". Valid reasons: ${VALID_REASONS.join(', ')}`,
    ...
    if (attr.cssVar) {
      if (cssVarMap.has(attr.cssVar)) {
        warnings.push({
          ...
          message: `cssVar "${attr.cssVar}" used by both "${cssVarMap.get(attr.cssVar)}" and "${attrName}"`,
...
function validateSchema(blockType) {
  ...
  const themeableCount = Object.values(schema.attributes || {}).filter(a => a.themeable).length;
