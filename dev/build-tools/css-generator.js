 ...
 * Prioritizes expanded schemas (with icon-panel macros expanded) over base schemas
 * @param blockType
 */
function loadSchema( blockType ) {
	// Try expanded schema first (contains expanded icon-panel attributes with responsive: true)
	...
	// Use expanded schema if it exists, otherwise fallback to base schema
	...
	try {
		...
		const fileName = schemaPath.endsWith( '-schema-autogenerated.json' )
			? `${ blockType }-schema-autogenerated.json`
			: `${ blockType }.json`;
		throw new Error( `Failed to parse schema ${ fileName }: ${ error.message }` );
	}
}
...
function loadStructureSchema( blockType ) {
	...
	try {
		...
		console.warn(
			`Warning: Failed to parse structure schema ${ blockType }-structure.json: ${ error.message }`
		);
		return null;
	}
}
...
function formatCssValue( defaultValue, unit, type, transformValue = null, units = null ) {
	...
	const effectiveUnit =
		unit || ( Array.isArray( units ) && units.length > 0 ? units[ 0 ] : null );
	...
	if ( type === 'object' && typeof defaultValue === 'object' ) {
		...
		if (
			...
			defaultValue.right !== undefined &&
			defaultValue.bottom !== undefined &&
			defaultValue.left !== undefined
		) {
		...
		if (
			( defaultValue.tablet !== undefined || defaultValue.mobile !== undefined ) &&
...
function getDecomposedEntries( attr ) {
	...
	const unitFallback =
		defaultValue && typeof defaultValue === 'object' && defaultValue.unit
			? defaultValue.unit
			: attr.unit;
...
function extractCssAttributes( schema, structure, blockType ) {
	...
	for ( const [ attrName, attr ] of Object.entries( schema.attributes ) ) {
		...
		if ( ! selector ) {
			throw new Error(
				`CSS generator: Could not resolve selector for "${ attrName }" (appliesTo: ${
					attr.appliesTo || 'n/a'
				}) in ${ blockType }. Ensure structure schema defines this element with className/cssSelector.`
			);
		}
		...
		cssAttrs.push( {
			...
			default: formattedValue,
			description: attr.description,
			elementId, // For grouping in new system
			responsive: attr.responsive === true, // Only true if explicitly set
			control: attr.control,
			rawDefault: attr.default,
			unit: attr.unit,
 ...
 * @param structure
 * @param resolvedSelectors
 * @param usedElementIds
 * @param blockType
 */
function validateStructureSelectors( structure, resolvedSelectors, usedElementIds, blockType ) {
	...
	Object.values( structure.elements ).forEach( ( element ) => {
		...
		if (
			...
			! Array.isArray( element.appliesStyles ) ||
			element.appliesStyles.length === 0
		) {
			return;
		}
...
function generateScssPartial( blockType, schema, structure ) {
	const { cssAttrs, structureSelectors, usedElementIds } = extractCssAttributes(
		schema,
		structure,
		blockType
	);
		...
		if ( hover.length > 0 ) {
			...
			for ( const attr of hover ) {
				...
				if ( decomposed ) {
					...
					for ( const entry of decomposed.entries ) {
						content += `    ${ entry.cssProperty }: ${ buildSideVar(
							attr,
							entry
						) };\n`;
					}
	...
	const addResponsiveBlock = ( device ) => {
		for ( const [ selector, states ] of Object.entries( grouped ) ) {
			...
			for ( const attr of base ) {
				...
				if ( decomposed ) {
					if ( ! decomposed.omitShorthand ) {
						content += `  ${ attr.property }: ${ buildResponsiveVar(
							attr,
							device
						) };\n`;
					}
					for ( const entry of decomposed.entries ) {
						content += `  ${ entry.cssProperty }: ${ buildResponsiveSideVar(
							attr,
							entry,
							device
						) };\n`;
 ...
 * @param options
 */
function isWarnOnly( options = {} ) {
	return (
		Boolean( options.warnOnly ) ||
...
async function generate( options = {} ) {
	const warnOnly = isWarnOnly( options );
		...
		for ( const blockType of BLOCKS ) {
			try {
				...
				if ( result ) {
					...
					const outputPath = path.join( ROOT_DIR, 'css', 'generated', fileName );
	...
	if ( results.errors.length > 0 ) {
		...
		console.log( '' );
		console.log(
			`⚠️  CSS generation completed with ${ results.errors.length } warning(s) (${ elapsed }ms)\n`
		);
		return results;
	}

	console.log(
		`✅ CSS generation: ${ results.success.length } files, ${ results.totalDeclarations } declarations (${ elapsed }ms)\n`
	);
	return results;
}
