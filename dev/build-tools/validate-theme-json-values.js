...
function collectAttributeInfo( getPropertyScale ) {
	...
	schemas.forEach( ( { data } ) => {
		...
		Object.entries( attrs ).forEach( ( [ attrName, attr ] ) => {
			const existing = info.get( attrName ) || {
				isScalar: false,
				requiresUnit: false,
				cssProperty: null,
			};
...
async function main() {
	...
	if ( files.length === 0 ) {
		console.log(
			'No theme JSON files provided. Set GUTPLUS_THEME_JSON_DIR or GUTPLUS_THEME_JSON_PATHS.'
		);
		process.exit( 0 );
	}
	...
	console.log(
		`${ colors.green }${ colors.bold }✓ Theme JSON values validated${ colors.reset }`
	);
}
