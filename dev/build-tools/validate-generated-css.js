...
function validateBlock(blockType) {
  ...
  if (!fs.existsSync(cssPath)) {
    return {
      errors: [{ message: `Missing generated CSS: ${cssPath}. Run npm run schema:build.` }],
      warnings: [],
    };
  }
  ...
  Object.entries(schema.attributes || {}).forEach(([attrName, attr]) => {
    ...
    if (isDecomposable) {
      ...
      if (missing.length > 0) {
        errors.push({
          message:
            `Setting "${attrName}" should emit side vars but they are missing in generated CSS: ${missing.join(', ')}.`,
      ...
      if (attr.responsive === true) {
        ...
        const responsiveMissing = responsiveExpected.filter((token) => !hasToken(cssContent, token));
