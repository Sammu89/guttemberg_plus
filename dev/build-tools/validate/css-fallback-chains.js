...
function validateLonghandFallback(property, value, baseProperty, blockType, hasShorthand) {
  ...
  const varMatch = parsed.mainVar.match(/--([a-z]+)-([a-z-]+?)-(top|right|bottom|left|top-left|top-right|bottom-left|bottom-right)$/);
  if (!varMatch) {
  ...
  if (baseProperty === 'margin' && !hasShorthand && parsed.fallback && !parsed.fallback.includes('var(')) {
...
function validateScssFile(blockType) {
  ...
  for (const line of lines) {
    ...
    const shorthandMatch = line.match(/^\s*(border-width|border-color|border-style|border-radius|padding|margin)\s*:/);
    if (shorthandMatch) {
  ...
  const propertyRegex = /^\s*(border-(?:top|right|bottom|left)-(?:width|color|style|radius)|padding-(?:top|right|bottom|left)|margin-(?:top|right|bottom))\s*:\s*(.+?);/;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (match) {
      ...
      if (DECOMPOSABLE_PROPERTIES[baseProperty]) {
        ...
        const result = validateLonghandFallback(property, value, baseProperty, blockType, hasShorthand);
        if (!result.valid) {
          errors.push({
            line: i + 1,
            property,
...
function validate() {
  ...
  console.log(`${colors.dim}Checking decomposed properties have proper fallback chains${colors.reset}\n`);
  ...
  for (const blockType of BLOCKS) {
    ...
    if (!result.exists) {
      console.log(`${colors.yellow}⚠${colors.reset}  ${blockType}: File not found (${result.file})`);
      continue;
    }
    ...
    if (result.errors.length === 0) {
      ...
      console.log(`${colors.red}✗${colors.reset} ${blockType}: ${result.errors.length} error(s)`);
      totalErrors += result.errors.length;

      for (const error of result.errors) {
        console.log(`  ${colors.dim}Line ${error.line}:${colors.reset} ${error.property}`);
  ...
  if (totalErrors > 0) {
    ...
    console.log(`When a side-specific CSS variable is undefined, it must fall back to the shorthand value.${colors.reset}\n`);
    return false;
  }

  console.log(`${colors.green}✓ All fallback chains validated successfully${colors.reset}\n`);
  return true;
}
