...
export default function Edit( { attributes, setAttributes, clientId } ) {
	 ...
	 * Icons are stored separately in accordion/tabs blocks (iconInactiveSource/iconActiveSource attributes)
	...
	const extractHeadingsFromBlocks = useCallback( ( blocks, currentClientId, options = {} ) => {
		...
		const processBlock = ( block ) => {
			...
			// Note: Icons are stored in iconInactiveSource/iconActiveSource attributes, not in title
			...
			// Note: Icons are stored in iconInactiveSource/iconActiveSource attributes, not in tab.title
	...
	const schemaDefaults = useMemo( () => {
		...
		return tocSchema.defaultValues || {};
	}, [] );
...
/* ========== AUTO-GENERATED-STYLES-END ========== */
	...
	const renderIcon = () => {
		...
		const inactiveSource = effectiveValues.iconInactiveSource || allDefaults.iconInactiveSource;
		const activeSource = effectiveValues.iconActiveSource || allDefaults.iconActiveSource;
		...
		if ( displaySource.kind === 'char' ) {
			iconElement = (
				<span className="toc-icon toc-icon-char">
					{ displaySource.value }
				</span>
			);
			...
			iconElement = (
				<span className="toc-icon toc-icon-library">
		...
		return (
			<span className="toc-icon-wrapper" aria-hidden="true">
				{ iconElement }
			</span>
		);
	};
 ...
 * @param attributes
 */
function renderHeadingsList( headings, attributes, defaults ) {
	...
	[ 'h1', 'h2', 'h3', 'h4', 'h5', 'h6' ].forEach( ( level ) => {
		const style =
			attributes[ `${ level }NumberingStyle` ] ??
			defaults?.[ `${ level }NumberingStyle` ];
		if ( style !== undefined ) {
			numberingStyles[ `--toc-${ level }-numbering` ] = style;
			numberingDataAttributes[ `data-${ level }-numbering` ] = style;
		}
	...
	return (
		<ul
			...
			data-base-level={ ( () => {
				...
				let minLevel = levels.length ? Math.min( ...levels ) : 1;
				const baseStyle =
					attributes.h1NumberingStyle ?? defaults?.h1NumberingStyle ?? 'decimal';
				if ( baseStyle === 'none' && minLevel <= 1 ) {
					minLevel = 2;
				}
				return minLevel;
			} )() }
