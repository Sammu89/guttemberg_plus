...
export default function Edit( { attributes, setAttributes, clientId } ) {
	...
	// Extract schema defaults from comprehensive schema (SINGLE SOURCE OF TRUTH!)
	const schemaDefaults = useMemo( () => {
		// Use pre-computed defaultValues from comprehensive schema
  ...
  // Extract object-type attributes with fallbacks
	...
	return {
		...
		titleText: {
			...
			fontWeight: (() => { const val = effectiveValues.titleFontWeight; if (val === null || val === undefined) return '600'; if (typeof val === 'string') return val; if (typeof val === 'number') return val; if (typeof val === 'object' && val.value !== undefined) { return `${val.value}${val.unit || ''}`; } return '600'; })(),
			textDecorationColor: (() => { const val = effectiveValues.titleDecorationColor; if (val === null || val === undefined) return 'currentColor'; if (typeof val === 'string') return val; if (typeof val === 'number') return val; if (typeof val === 'object') { if (val.color !== undefined) return val.color; if (val.value !== undefined) { return `${val.value}${val.unit || ''}`; } } return 'currentColor'; })(),
			...
			letterSpacing: (() => { const val = effectiveValues.titleLetterSpacing; if (val === null || val === undefined) return 'normal'; if (typeof val === 'string') return val; if (typeof val === 'number') return val; if (typeof val === 'object' && val.value !== undefined) { return `${val.value}${val.unit || ''}`; } return 'normal'; })(),
			...
			textDecorationLine: (() => { const val = effectiveValues.titleTextDecoration; if (val === null || val === undefined) return 'none'; if (typeof val === 'string') return val; if (typeof val === 'number') return val; if (typeof val === 'object' && val.value !== undefined) { return `${val.value}${val.unit || ''}`; } return 'none'; })(),
			lineHeight: (() => { const val = effectiveValues.titleLineHeight; if (val === null || val === undefined) return '1.4'; if (typeof val === 'string') return val; if (typeof val === 'number') return val; if (typeof val === 'object') { const deviceVal = val[responsiveDevice]; if (deviceVal !== undefined) { if (typeof deviceVal === 'string') return deviceVal; if (typeof deviceVal === 'number') return deviceVal; if (typeof deviceVal === 'object' && deviceVal.value !== undefined) { return `${deviceVal.value}${deviceVal.unit || ''}`; } return deviceVal; } if (val.value !== undefined) { return `${val.value}${val.unit || ''}`; } } return '1.4'; })(),
...
/* ========== AUTO-GENERATED-STYLES-END ========== */
	...
	const titleFormattingStyles = undefined;
	const titleTextInlineStyles = styles.titleText;
