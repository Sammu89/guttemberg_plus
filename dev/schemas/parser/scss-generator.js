...
function shouldEmitCSS(attr) {
  ...
  if (attr.type === 'composite') return false;
  if (attr.outputsCSS === false) return false;
...
function normalizeDefaultValue(attr) {
  ...
  if (attr.cssValueMap && Object.prototype.hasOwnProperty.call(attr.cssValueMap, attr.default)) {
...
function buildVarChain(attr, device, blockType) {
  ...
  const baseVar = attr.cssVar || buildCssVarName(blockType, attr.element, attr.cssProperty, state);
...
function generateRootVariables(attributes) {
  ...
  const entries = Object.entries(attributes || {})
    .filter(([, attr]) => attr && attr.cssVar && attr.type !== 'composite' && attr.outputsCSS !== false)
    .sort((a, b) => {
