 ...
 * @returns {Object} Structured representation of the template
 */
function parseHTMLTemplate(htmlFilePath) {
  ...
  function handleTemplateDirective(node, elements, variations, slots, conditions) {
    ...
    if (whenAttr || elseAttr) {
      ...
      if (elseAttr && conditions.length > 1) {
        ...
        if (prevCondition.type === 'conditional' && prevCondition.expression && !prevCondition.falseContent) {
    ...
    if (switchAttr) {
      ...
      const caseNodes = contentDOM.window.document.querySelectorAll('template[data-case], template[data-default]');
    ...
    if (defaultAttr && node.parentElement && node.parentElement.hasAttribute('data-switch')) {
      const switchExpr = node.parentElement.getAttribute('data-switch');
      if (!variations[switchExpr]) {
        variations[switchExpr] = {
          type: 'switch',
  ...
  function traverse(node, parentId = null) {
    ...
    if (elId) {
      const classes = node.getAttribute('class')?.split(' ').filter(c => c) || [];
  ...
  const templateRoot = document.querySelector('[data-el]')?.closest('div') || document.body.firstElementChild;
  if (templateRoot) {
 ...
 * @returns {Object} Parsed structures keyed by block type
 */
