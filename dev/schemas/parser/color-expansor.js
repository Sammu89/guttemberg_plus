...
function expandColorPanelMacro(macroName, macro, blockType) {
  ...
  const fields = Array.isArray(macro.fields) && macro.fields.length ? macro.fields : DEFAULT_FIELDS;
  ...
  const inferredBase = `${blockType}-${baseName.replace(/Colors?$/, '')}`.replace(/_/g, '-');
