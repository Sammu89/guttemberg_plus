...
function addCompositeAttributes(attributes) {
  ...
  Object.entries(attributes || {}).forEach(([attrName, attr]) => {
    ...
    const borderMatch = /^border-(top|right|bottom|left)-(width|style|color)$/.exec(attr.cssProperty);
    if (borderMatch) {
    ...
    const radiusMatch = /^border-(top-left|top-right|bottom-right|bottom-left)-radius$/.exec(attr.cssProperty);
    if (radiusMatch) {
