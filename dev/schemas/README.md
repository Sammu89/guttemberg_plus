...
Document the **current** schema pipeline and the comprehensive pipeline in progress. The goal is still a single source of truth, but today the build is split between the schema compiler (JS/TS/PHP + edit/save injections) and the HTML parser + comprehensive SCSS generator (structure mapping + CSS variable SCSS).

---

## Current Build Snapshot (schema-compiler)

- Source of truth for build-time outputs: `schemas/{block}.json` (camelCase attributes, `cssVar` + `cssProperty` define CSS mapping).
- `npm run schema:build` runs `build-tools/schema-compiler.js` to expand icon panels, write `schemas/{block}-schema-autogenerated.json`, generate CSS var mappings + style builders, and inject auto-generated code into `blocks/*/src/edit.js` + `blocks/*/src/save.js`.
- JSX structure is generated from `schemas/{block}-structure-mapping-autogenerated.json` (if present). That mapping is produced by the HTML parser CLI (`schemas/parser/main-orchestrator.js`) from `schemas/{block}-structure.html`.
- Comprehensive SCSS variables in `css/generated/{block}_variables.scss` are produced by the comprehensive parser + `schemas/parser/scss-generator.js` when run manually (not part of `schema:build` yet).

---

## Target Principles (Comprehensive Pipeline)

### 1. **Single Source of Truth (goal)**
One comprehensive schema should drive CSS, edit.js, save.js, and mappings. Today, `schema-compiler` still reads the minimal schema, while the comprehensive parser powers structure mapping + SCSS variables.

### 2. **Kebab-Case Attributes (comprehensive pipeline)**
Comprehensive schema attributes are kebab-case to align with CSS vars:
```javascript
"title-icon-size"  → --accordion-title-icon-size
"border-top-color" → --accordion-border-top-color
```
Note: current source schemas still use camelCase (e.g., `titleColor`) and map to CSS vars via `cssVar`.

### 3. **Auto-Generated CSS Variables (comprehensive pipeline)**
...
- CSS vars auto-generated or auto-inferred
- UI ordering driven by groups + `order` (current), or `panelId` (comprehensive pipeline)

## Status + Next Phase

- Structure mapping is generated from `schemas/{block}-structure.html` and exported to `schemas/{block}-structure-mapping-autogenerated.json` (used by edit/save JSX generation).
- CSS variable SCSS is generated in `css/generated/{block}_variables.scss` by `schemas/parser/scss-generator.js`.
- `schema:build` currently uses `build-tools/schema-compiler.js` to generate JS/TS/PHP/config artifacts and inject edit/save code.
- Next phase: replace the remaining schema-compiler pipelines with the comprehensive schema outputs so edit.js/save.js/sidebar panels can be driven entirely by the comprehensive schema.

---

## Architecture Flow (Current Build)
...
schemas/{block}.json
        ...
        │   ├─ shared/src/styles/{block}-css-vars-generated.js
        │   ├─ shared/src/styles/{block}-frontend-css-vars-generated.js
        │   ├─ blocks/{block}/src/{block}-attributes.js
        │   └─ inject edit.js/save.js (STYLES + JSX markers)
        │
schemas/{block}-structure.html
        │
        └─ schemas/parser/html-parser.js (via main-orchestrator CLI)
            └─ schemas/{block}-structure-mapping-autogenerated.json
                └─ build-tools/generators/structure-jsx-generator.js (injected into edit/save)
```

## Architecture Flow (Comprehensive Pipeline - WIP)
...
│  │ accordion.json   │  │  accordion-structure.html │   │
│  └──────────────────┘  └──────────────────────┘       │
└────────────┬──────────────────┬────────────────────────┘
             │                  │
             └────────┬─────────┘
        ...
        └─────────────┬───────────────┘
                      ↓
┌─────────────────────────────────────────────────────────┐
│         COMPREHENSIVE SCHEMA (Generated)                │
│  • ALL attributes (macro-expanded, kebab-case)          │
│  • ALL CSS variables (auto-generated)                   │
│  • ALL responsive variants (tablet/mobile)              │
│  • ALL defaults                                         │
│  • Structure mapping exported separately (elements map) │
│  • cssVarMap (CSS var → attribute metadata)             │
│  • selectorVarMap (selector → CSS vars array)           │
...
This section describes the **comprehensive parser** in `schemas/parser` (WIP). The current build uses `build-tools/schema-compiler.js` instead.
...
- `border-radius`: 1–4-value shorthand (no ellipse support).
...
- ✅ edit.js generation (knows all variations)
- ✅ save.js generation (knows DOM structure)
- ✅ SCSS generation (knows selectors and hierarchy)
...
#### Current build outputs (schema-compiler)
...
- `shared/src/config/css-var-mappings-generated.js`: attribute → CSS var map (JS).
- `php/css-defaults/css-mappings-generated.php`: attribute → CSS var map (PHP).
...
#### Comprehensive pipeline outputs (WIP)
...
- `css-var-mappings-generated.js` exposes:
  - `getCssVarName(attrName, blockType)` (attribute → CSS var)
...
### Current schema attribute example (schema-compiler)
```json
"titleColor": {
  "type": "color-panel",
  "appliesTo": "title",
...
### Comprehensive (WIP) atomic attribute example
```json
"title-color": {
  // Type & Control
  "type": "string",
...
### Comprehensive (WIP) panel-expanded attribute example
```json
"title-icon-size": {
  "type": "string",
  "control": "SliderWithInput",
...
- Panels are driven by `schema.tabs` → groups (id/title/order) and optional subgroups.
...
- Composite panel types are **object-valued attributes** that `ControlRenderer` expands into multiple sub-controls:
  - `type: "color-panel"` → text/background + hover states
  - `type: "box-panel"` → border/radius/shadow/padding/margin
  - `type: "typography-panel"` → font family/size/line-height
  - `type: "border-panel"` → width/color/style (single-side use cases)
...
## Build Pipeline (Current: schema-compiler)
...
### What It Does (schema-compiler.js)
```javascript
...
### Comprehensive Parser CLI (Optional/WIP)
```bash
...
### Current Build (schema-compiler)

#### `build-tools/schema-compiler.js` ⭐ CURRENT ORCHESTRATOR
...
### Comprehensive Parser (WIP)

#### `html-parser.js`
Parses HTML template files into structured data.

**Features:**
- Extracts `data-el` → element IDs
- Extracts `class` → selectors
- Parses `data-when`/`data-else` → conditionals
- Parses `data-switch`/`data-case` → variations
- Parses `<template data-slot>` → reusable fragments
...
#### `main-orchestrator.js` ⭐ COMPREHENSIVE ORCHESTRATOR
The brain of the comprehensive pipeline.

**Does:**
1. Expands macros → atomic attributes
2. Auto-generates cssVar names using formula
3. Applies responsive variants (tablet/mobile)
4. Builds defaultValues map
5. Builds cssVarMap (CSS var → attribute metadata)
6. Builds selectorVarMap (selector → CSS vars array)
...
Expands `typography-panel` → per-field × per-state attributes
...
#### `border-expander.js`
Expands `border-panel` → 12 attributes (4 sides × 3 properties)
- Width, color, style
- Per side: top, right, bottom, left

#### `box-expander.js`
Expands `box-panel` → 25+ attributes
...
✅ **Zero synchronization issues** - One source drives everything
✅ **Zero manual edits** - Add attribute → all files auto-update
✅ **Auto-generated CSS vars** - No manual naming, always consistent
✅ **Trivial debugging** - Follow comprehensive schema to understand everything
✅ **Trivial new features** - Define macro → run build → done
✅ **Clean structure** - No redundancy, each section has clear purpose
✅ **Panel/standalone distinction** - Clear organization in sidebar
✅ **Edge cases handled** - Responsive + state + per-side all work

---

## Adding a New Feature

**Current build note:** For `schema-compiler`, add the attribute to `schemas/{block}.json` (with `cssVar`, `cssProperty`, and `appliesTo`), then run `npm run schema:build`. New control types require updates in `ControlRenderer` and the generator(s) if they need special CSS-var expansion.
...
A: This is for the comprehensive pipeline (WIP). The current source schemas are camelCase.

**Q: Why auto-generate cssVar names?**
A: In the current pipeline, `schema-compiler` auto-infers `cssVar` for panel types if missing; the comprehensive pipeline uses a formula for all attrs.
