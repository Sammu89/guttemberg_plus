...
function validateStateSelectors( comprehensiveSchema ) {
	const errors = [];
	...
	Object.entries( attributes || {} ).forEach( ( [ attrName, attr ] ) => {
		if ( ! shouldEmitCSS( attr ) || ! attr.element ) {
			return;
		}

		const cssVar = attr.cssVar || '';
		Object.entries( stateSuffixes ).forEach( ( [ suffix, state ] ) => {
			// Check if the suffix appears at the end or before a device suffix (tablet/mobile)
			const hasStateSuffix = cssVar.endsWith( suffix ) ||
				cssVar.endsWith( `${ suffix }-tablet` ) ||
				cssVar.endsWith( `${ suffix }-mobile` );

			if ( hasStateSuffix && attr.state !== state ) {
				errors.push(
					`[${ blockType }] Attribute "${ attrName }" uses ${ state } cssVar "${ cssVar }" ` +
						`but is missing state: "${ state }"`
				);
			}
		} );
		...
		if ( attr.state === 'hover' ) {
			...
			elementIds.forEach( ( elementId ) => {
				...
				const selector = buildSelector( element.selector, 'hover', 'desktop', blockType );
				if ( ! selector.includes( ':hover' ) ) {
					errors.push(
						`[${ blockType }] Hover state for "${ attrName }" did not generate :hover selector (${ selector })`
					);
				}
			} );
		}
