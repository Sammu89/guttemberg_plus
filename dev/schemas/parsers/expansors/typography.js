...
const DEFAULT_FIELDS = [ 'fontFamily', 'fontSize', 'lineHeight' ];
...
const FIELD_DEFS = {
	...
	formatting: {
		...
		description: 'Bold, italic, underline, overline, line-through)',
		outputsCSS: false,
		supportsState: false,
	},
	noLineBreak: {
	...
	textDecoration: {
		...
		type: 'string',
		control: 'FormattingControl',
		cssVarSuffix: 'text-decoration-line',
...
function expandTypographyPanelMacro( macroName, macro, blockType ) {
	...
	states.forEach( ( state ) => {
		const stateSuffix =
			state === 'base' ? '' : state.charAt( 0 ).toUpperCase() + state.slice( 1 );
		const stateDefaults =
