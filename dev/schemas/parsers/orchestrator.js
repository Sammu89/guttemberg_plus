...
const { expandIconPanelMacro } = require( './expansors/icon' );
const { expandTypographyPanelMacro } = require( './expansors/typography' );
const { expandBorderPanelMacro } = require( './expansors/border' );
const { expandBoxPanelMacro } = require( './expansors/box' );
const { expandColorPanelMacro } = require( './expansors/color' );
const { addCompositeAttributes } = require( './expansors/composite' );
const { applyResponsiveVariants } = require( './expansors/responsive' );
 ...
 * @return {Object} Complete comprehensive schema
 */
function createComprehensiveSchema( mergedSchema ) {
	...
	console.log(
		`[Comprehensive Expander] Total attributes after expansion: ${
			Object.keys( expandedAttributes ).length
		}`
	);
	...
	Object.entries( withComposites ).forEach( ( [ attrName, attr ] ) => {
		...
		const selectors = elementIds.map( ( id ) => elements[ id ]?.selector ).filter( Boolean );
	...
	console.log(
		`  - Total responsive selectors: ${ comprehensive._meta.totalResponsiveSelectors }`
	);
 ...
 * @return {Array} Responsive selectors array
 */
function buildResponsiveSelectors( attributes, elements, blockType ) {
	...
	Object.entries( attributes ).forEach( ( [ attrName, attr ] ) => {
		if ( ! attr.responsive || ! attr.element || ! attr.cssProperty ) {
			return;
		}
 ...
 * @return {Object} Comprehensive schemas keyed by block type
 */
...
function writeComprehensive( outputPath, comprehensiveSchema ) {
	...
	if ( structure ) {
		const baseName =
			blockType || path.basename( outputPath, '.json' ).replace( /-comprehensive$/, '' );
		const mappingDir = path.join( ROOT_DIR, 'schemas' );
		fs.mkdirSync( mappingDir, { recursive: true } );
		const mappingPath = path.join(
			mappingDir,
			`${ baseName }-structure-mapping-autogenerated.json`
		);
...
function runCli() {
	...
	if ( args.length === 0 ) {
		...
		console.log(
			'  node parser/comprehensive-expander.js schemas/accordion.json --out output/accordion-comprehensive.json'
		);
		process.exit( 1 );
	}
